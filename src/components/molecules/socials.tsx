...
export default function Socials() {
  ...
  return (
    ...
    >
      <div className="flex items-center bg-white/10 dark:bg-white/10 backdrop-blur-md border border-white/20 dark:border-white/20 rounded-full px-4 py-3 shadow-2xl">
          ...
          return (
            <Link
              ...
              style={{
                ...
                marginLeft: isHovered ? '16px' : '2px',
                marginRight: isHovered ? '16px' : '2px',
            ...
            >
              <div 
                className={`flex items-center justify-center w-11 h-11 rounded-full transition-colors duration-300 ${
                  isHovered && 'bg-white/20 dark:bg-white/20'
                }`}
              >
                <Icon 
        ...
        {/* Divider */}
        <div className="w-px h-7 bg-gray-300/30 dark:bg-white/20 mx-2 flex-shrink-0" />
        ...
        <button
          ...
          style={{
            ...
            marginLeft: hoveredIndex === footerInfo.socials.length ? '16px' : '2px',
            marginRight: hoveredIndex === footerInfo.socials.length ? '16px' : '2px',
            transition: 'margin 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)'
          }}
        >
          <div 
            className={`flex items-center justify-center w-11 h-11 rounded-full transition-colors duration-300 ${
              hoveredIndex === footerInfo.socials.length && 'bg-white/20 dark:bg-white/20'
            }`}
          >
            {isDark ? (
              <Sun 
                className={`h-5 w-5 transition-colors duration-300 ${
                  hoveredIndex === footerInfo.socials.length
                    ? 'text-black dark:text-white' 
                    : 'text-gray-700 dark:text-gray-300'
                }`}
                strokeWidth={1.5}
